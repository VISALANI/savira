...
export default function EditProductPage() {
    ...
    adminAPI.getProduct(id)
      .then((res) => {
        ...
        setForm({
          ...
          category: p.category, fabric: p.fabric, stock: String(p.stock),
          washCare: p.washCare, estimatedDelivery: p.estimatedDelivery,
          codAvailable: p.codAvailable, isNewArrival: p.isNewArrival,
          isBestSeller: p.isBestSeller, isFeatured: p.isFeatured,
  ...
  const toggleSize = (s: string) =>
    setForm((f) => ({ ...f, sizes: f.sizes.includes(s) ? f.sizes.filter((v) => v !== s) : [...f.sizes, s] }));
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="font-playfair text-2xl text-charcoal">Edit Product</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div>
            ...
            <textarea required value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={4} className="input-field resize-none" />
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" min="0" value={form.originalPrice} onChange={(e) => setForm({ ...form, originalPrice: e.target.value })} className="input-field" />
            </div>
            <div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div>
            ...
            <input value={form.colors} onChange={(e) => setForm({ ...form, colors: e.target.value })} className="input-field" placeholder="Sage Green, Ivory, Dusty Rose" />
          </div>
        </div>

        <div className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
          <h2 className="font-playfair text-base text-charcoal">Images</h2>
          {imageUrls.map((url, i) => (
            <div key={i} className="flex gap-2">
              <input value={url} onChange={(e) => { const u = [...imageUrls]; u[i] = e.target.value; setImageUrls(u); }} className="input-field flex-1" placeholder="Image URL" />
              {url && <img src={url} alt="" className="w-12 h-14 rounded-lg object-cover bg-gray-100" onError={(e) => (e.currentTarget.style.display = "none")} />}
              {imageUrls.length > 1 && (
                <button type="button" onClick={() => setImageUrls(imageUrls.filter((_, j) => j !== i))} className="text-gray-300 hover:text-red-400"><FiX size={16} /></button>
              )}
            </div>
          ))}
